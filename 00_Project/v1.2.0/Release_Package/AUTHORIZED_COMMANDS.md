# Authorized Command Drafts

These commands are not authorized until the product owner explicitly approves commit execution.
Do not run them before approval. Do not use `git add .` or `git add -A`.
...
git commit -m "feat(runtime): integrate v1.1 office world simulation"
```
...
git commit -m "feat(office): add interactive office hub and resident actions"
```
...
git commit -m "feat(content): integrate office dialogue story rumor and events"
```
...
git add \
  server.js
git commit -m "fix(release): stabilize static asset fallback and interaction safety"
```
...
git commit -m "test(rc): add release candidate acceptance coverage"
```
...
git commit -m "docs(release): prepare v1.2.0 release candidate handoff"
```

## Forbidden

```bash
# Forbidden for this handoff
git add .
git add -A
git push --force
git tag -f v1.2.0-rc.1
git switch main
git merge main
railway up
railway deploy
